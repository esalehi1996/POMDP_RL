...
class DroneSurveillanceEnv(gym.Env):
    def __init__(self):
        ...
        self.state_space = spaces.Discrete(
            self.map_size[0] * self.map_size[1] * (self.map_size[0] * self.map_size[1] - 2))
    ...
    def step(self, action):
        ...
        for gb_action in range(5):
            if gb_action == 0:
                if (ground_bot_row != (self.map_size[0] - 1)) and not (
                        (ground_bot_row == (self.map_size[0] - 2)) and (ground_bot_col == (self.map_size[1] - 1))):
                    viable_next_states.append(
                        self.encode_state(drone_row, drone_col, ground_bot_row + 1, ground_bot_col))
            elif gb_action == 1:
                if (ground_bot_col != (self.map_size[1] - 1)) and not (
                        (ground_bot_row == (self.map_size[0] - 1)) and (ground_bot_col == (self.map_size[1] - 2))):
                    viable_next_states.append(
                        self.encode_state(drone_row, drone_col, ground_bot_row, ground_bot_col + 1))
            elif gb_action == 2:
        ...
        next_state = viable_next_states[
            self.np_random.multinomial(1, np.ones(len(viable_next_states)) / float(len(viable_next_states))).argmax()]
    ...
    def reset(self):
        ...
        g_encoding = g_initlist[
            self.np_random.multinomial(1, np.ones(len(g_initlist)) / float(len(g_initlist))).argmax()]
        ground_bot_col = (g_encoding + 1) % self.map_size[1]
        ground_bot_row = (g_encoding + 1) // self.map_size[1]
    ...
    def render(self, mode=None):
        ...
        circle(screen, blue, (starting_col + drone_col * col_steps, rows - (starting_row + drone_row * row_steps)),
               largedot_size)  # plotting drone
        circle(screen, red,
               (starting_col + ground_bot_col * col_steps, rows - (starting_row + ground_bot_row * row_steps)),
               mediumdot_size)  # plotting ground_bot
        for i in range(self.map_size[0]):
            for j in range(self.map_size[1]):
                circle(screen, black, (starting_col + j * col_steps, rows - (starting_row + i * row_steps)),
                       smalldot_size)  # plotting grid
