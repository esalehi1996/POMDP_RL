...
class RockSamplingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            assert False, 'Invalid action given: `{}`. Action should be from 0-{}'.format(action,
                                                                                          5 + self.num_rocks - 1)
    ...
    def render(self, mode=None):
        ...
        circle(screen, blue, (starting_col + rover_col * col_steps, rows - (starting_row + rover_row * row_steps)),
               largedot_size)  # plotting rover
        for i, rp in enumerate(self.rock_positions):
            ...
            if rs:
                circle(screen, green,
                       (starting_col + rock_pos_col * col_steps, rows - (starting_row + rock_pos_row * row_steps)),
                       mediumdot_size + 2)  # good rock
            else:
                circle(screen, red,
                       (starting_col + rock_pos_col * col_steps, rows - (starting_row + rock_pos_row * row_steps)),
                       mediumdot_size + 2)  # bad rock
            circle(screen, grey,
                   (starting_col + rock_pos_col * col_steps, rows - (starting_row + rock_pos_row * row_steps)),
                   mediumdot_size)  # plotting rocks
        ...
        if self.last_action >= 5:
            ...
            rock_pos_row = rp // self.map_size[1]
            rock_pos_col = rp % self.map_size[1]
            line(screen, black, (starting_col + rover_col * col_steps, rows - (starting_row + rover_row * row_steps)),
                 (starting_col + rock_pos_col * col_steps, rows - (starting_row + rock_pos_row * row_steps)))
